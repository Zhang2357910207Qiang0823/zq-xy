	...
	$(window).scroll(function(e) {
		...
		if(sTop >= navPos) {
			...
			if(nav.hasClass('fixed')) {
				...
				$('.banner').css('margin-bottom',0);//不浮动时，删除占位
			}
		}
		...
		if(sTop >= introPos.start && sTop < introPos.end) {
			if(!($('.intro').hasClass('active'))) {
				$('.intro').addClass('active');//添加高亮
			}
		} else {
			if($('.intro').hasClass('active')) {
				$('.intro').removeClass('active');//删除高亮
			}
		}
		...
		if(sTop >= usagePos.start && sTop < usagePos.end) {
			if(!($('.usage').hasClass('active'))) {
				$('.usage').addClass('active');//添加高亮
			}
		} else {
			if($('.usage').hasClass('active')) {
				$('.usage').removeClass('active');//删除高亮
			}
		}
		...
		if(sTop >= choicePos.start && sTop < choicePos.end) {
			if(!($('.choice').hasClass('active'))) {
				$('.choice').addClass('active');//添加高亮
			}
		} else {
			if($('.choice').hasClass('active')) {
				$('.choice').removeClass('active');//删除高亮
			}
		}
